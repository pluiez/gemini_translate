...
sequenceDiagram
    ...
    User->>GradioUI: Select "Rewrite" mode
    GradioUI->>RunPy: Update processing mode
    ...
    GenReq->>GenReq: generate_rewrite_requests()
    GenReq-->>RunPy: Return rewriting requests
    ...
    loop For each request
        alt Cache hit
            ...
            ConcAPI->>APIClient: Send rewrite request to Client.get()
            APIClient->>LLM: Send request with rewrite prompt
            LLM-->>APIClient: Return rewritten content
    ...
    RunPy->>RunPy: Update inputs with rewritten content
    RunPy->>RunPy: Create CSV files with rewritten content
    ...
    GradioUI-->>User: Download ZIP with rewritten files
``` 